...
updated: 2026-07-28T01:18:02.972Z
external: null
---
...
- 2026-07-28T01:18:02.972Z Closed out fully in 6a83c82. TasksListView now has the selection state it was missing: per-row checkboxes (hidden until hover or selection, so an affordance most visits never use does not put an empty box on every row; stopPropagation because selecting and opening are different intents) and a bulk bar that appears only once something is ticked. The bar names how many of the selection are actually READY rather than just how many are selected — selecting a blocked task is normal, and a button promising to dispatch it would be lying. Dispatch routes through the same DispatchDialog the board uses; it was already selection-source-agnostic and needed no changes. NOT done, and worth a separate task if wanted: set-priority and move-to-an-epic across the selection. Both are straightforward now that selection exists — they just need the bulk-update endpoints, which do not exist yet (handleUpdate is per-task).
