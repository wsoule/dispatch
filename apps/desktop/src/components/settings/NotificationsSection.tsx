...
import { NOTIFICATION_KINDS } from '@dispatch/core/browser';
...
export function NotificationsSection({
  ...
  useEffect(() => {
    setWebhook(config.notifications.webhook ?? '');
  }, [config]);
          ...
          <Field className="gap-1.5">
            ...
            <Input
              ...
              onBlur={() => {
                ...
                if (next !== (config.notifications.webhook ?? '')) {
                  // Empty clears the key rather than storing an empty URL.
                  void onSave({
                    notifications: { webhook: next === '' ? null : next },
                  });
                }
              }}
