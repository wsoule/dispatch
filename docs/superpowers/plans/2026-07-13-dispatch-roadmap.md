...
| 6. Hardening + release   | author at phase start                    | Reconciliation edge cases, doctor completeness, docs, **Tauri bundling + `bun build --compile` dispatchd sidecar binary**, Rust CI, versioning, **licensing gate resolution**.                                                          | ✅ Complete (merged 2026-07-20): CI (bun + desktop + Rust jobs), doctor completeness (timestamps, non-epic parents, dependency cycles), compiled dispatchd sidecar (`bun run build:sidecar`, boots green), `docs/RELEASE-CHECKLIST.md`. Actual signing/publishing deliberately manual — see checklist; **Relay licensing grant remains the release blocker** |
