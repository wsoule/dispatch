...
Each phase ships working, independently useful software and gets its own detailed
implementation plan, authored when the phase starts (so it plans against real
interfaces, not guesses). Phase order matches spec §9.

| Phase | Plan file | Delivers | Status |
|---|---|---|---|
| 1. Core + CLI tracker | `2026-07-13-phase-1-core-cli-tracker.md` | `@dispatch/core` (task files, IDs, graph, store) + `dispatch` CLI (init/create/list/show/edit/status/next/doctor). A usable git-native tracker. | **Planned — ready to execute** |
...
| 4. Orchestrator MVP | `phase-4-orchestrator-mvp.md` (author at phase start) | Worktree manager, Executor interface + Claude Agent SDK executor, run lifecycle + normalized log streaming, approvals, review view (diff, merge/discard/request-changes). | Pending |
| 5. Planner + parallelism | `phase-5-planner-parallelism.md` (author at phase start) | Plan-confirm flow (prompt → structured epic/tasks proposal), epic ready-queue dispatch with concurrency limit, PR flow + polling. | Pending |
