...
// Reproduces `carto init`'s MCP auto-wiring, which has no opt-out flag: it
// merges a `carto` entry into every editor config it detects, in the user's
// home as well as the project. Paths are baked into the stub's script text
// because the stub is a real subprocess and inherits the real HOME.
function writeWiringCartoBinary(
  binDir: string,
  fakeHome: string,
  entry = '{"mcpServers":{"carto":{"command":"carto","args":["serve"]}}}'
): { path: string; version: string } {
  const write = (path: string): string =>
    `mkdir -p "$(dirname '${path}')" && cat > '${path}' <<'WIRED'\n${entry}\nWIRED`;
  return writeStubCartoBinary(
    binDir,
    [
      'mkdir -p .carto',
      'echo \'{"output":"AGENTS.md"}\' > .carto/config.json',
      'touch .carto/carto.db',
      write(join(fakeHome, '.cursor', 'mcp.json')),
      write(
        join(
          fakeHome,
          'Library',
      ...
      write(join(fakeHome, '.config', 'Claude', 'claude_desktop_config.json')),
      `mkdir -p "$(dirname '${join(fakeHome, '.codex', 'config.toml')}')" && printf '\\n[mcp_servers.carto]\\ncommand = "carto"\\n' >> '${join(fakeHome, '.codex', 'config.toml')}'`,
      write('.mcp.json'),
...
// carto's Claude Desktop path is platform-specific; the stub writes both the
// macOS and Linux locations so this suite asserts on whichever one applies.
...
// Dispatch runs carto to build a dependency index, not to reconfigure the
// user's editors — so `carto init`'s MCP auto-wiring must leave no trace,
// the same containment AGENTS.md already gets.
describe('cartoInit MCP wiring containment', () => {
  it('restores a global MCP config carto init rewrote', () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
      const cursor = join(home, '.cursor', 'mcp.json');
      const original = '{\n  "mcpServers": {\n    "mine": {}\n  }\n}\n';
      mkdirSync(join(home, '.cursor'), { recursive: true });
      writeFileSync(cursor, original);
      const binary = writeWiringCartoBinary(join(root, 'bin'), home);

      expect(cartoInit(root, binary, home).ok).toBe(true);
  ...
  it('removes a global MCP config carto init created from nothing', () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
      const binary = writeWiringCartoBinary(join(root, 'bin'), home);
      ...
      expect(existsSync(claudeDesktopConfig(home))).toBe(false);
    } finally {
      rmSync(root, { recursive: true, force: true });
      rmSync(home, { recursive: true, force: true });
    }
  ...
  it("restores Codex's config.toml, which carto appends to rather than merges", () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
  ...
  // .mcp.json is Dispatch's own: `dispatch init` writes the dispatch server
  // there before carto ever runs. A carto entry merged in would also hand
  // agents an MCP server the version gate in @dispatch/server withholds.
  it("restores the project's .mcp.json, which Dispatch itself owns", () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
      ...
      expect(cartoInit(root, binary, home).ok).toBe(true);
      expect(readFileSync(mcp, 'utf8')).toBe(original);
    } finally {
      rmSync(root, { recursive: true, force: true });
      rmSync(home, { recursive: true, force: true });
    }
  ...
  it('leaves behind no .vscode directory carto init created', () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
      const binary = writeWiringCartoBinary(join(root, 'bin'), home);
  ...
  // Containment must not cost the index: the container carto builds is the
  // whole point of running it.
  it('still reports the container carto built', () => {
    const root = mkdtempSync(join(tmpdir(), 'dispatch-carto-'));
    const home = mkdtempSync(join(tmpdir(), 'dispatch-carto-home-'));
    try {
      const binary = writeWiringCartoBinary(join(root, 'bin'), home);
      const result = cartoInit(root, binary, home);
      expect(result.ok).toBe(true);
