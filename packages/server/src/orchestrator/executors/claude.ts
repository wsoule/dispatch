...
// found anywhere — see rewriteMissingCliError. The native installer drops
// `claude` into a location (`~/.local/bin`) the desktop app's spawned daemon
// PATH already searches, so once the user runs this and re-dispatches, the
// PATH fallback in openQuery() picks it up with no further configuration.
...
function isMissingCliError(message: string): boolean {
  return /Native CLI binary for/.test(message);
}

// Rewrites the SDK's opaque missing-CLI message — meaningless to a desktop-app
// user who never touched npm or the SDK — into a concrete install command. Any
// other error passes through unchanged, so unrelated startup failures are
// never hidden behind a misleading "install Claude Code" hint.
function rewriteMissingCliError(message: string): string {
...
export class ClaudeExecutor implements Executor {
  ...
  // robustly. `query()` resolves that CLI *synchronously* and throws right
  // here when it can't find one — the exact failure that used to escape as an
  // opaque 500 and leave a run stuck 'running'. Resolution order:
  //
  ...
  //
  // If none of these yields a CLI, the SDK's raw
  // reinstall-the-npm-package message is rewritten into an actionable install
  // command; the orchestrator's startAndRegister catches this throw and marks
  // the run failed carrying exactly that text, which the UI surfaces on the
  // run instead of hanging on 'running'.
  private openQuery(prompt: MessageQueue, options: Options): Query {
    ...
    if (override !== undefined && override !== '') {
      try {
        return this.queryFn({ prompt, options: withExecutable(override) });
      } catch (err) {
    ...
    try {
      return this.queryFn({ prompt, options });
    } catch (err) {
      ...
      try {
        return this.queryFn({ prompt, options: withExecutable(onPath) });
      } catch (retryErr) {
