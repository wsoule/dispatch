...
// which builds packages/server's own upstream dependencies — but web isn't
// one of them (web depends on server's types, not the reverse), so it isn't
// in server's build graph at all. A server-only test run (`moonx server:test`,
// or `moon run :test --affected` when nothing touched web) can't assume
// packages/web/dist already exists. Building it here, once, up front makes
// this test self-sufficient either way; 120s covers a cold vite build plus
